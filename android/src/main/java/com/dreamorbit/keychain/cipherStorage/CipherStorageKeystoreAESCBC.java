...
public class CipherStorageKeystoreAESCBC implements CipherStorage {
        ...
        try {
            ...
            try {
                ...
                if (retry) {
                    ...
                    return encrypt(service, key, value, level);
                } else {
                    throw ex;
                }
            }

            byte[] encryptedUsername = encryptString(key, service, key);
            byte[] encryptedPassword = encryptString(key, service, value);
    ...
    public DecryptionResult decrypt(@NonNull String service, @NonNull byte[] key, @NonNull byte[] value) throws CryptoFailedException {
        ...
        try {
            ...
            String decryptedUsername = decryptBytes(key, key);
            String decryptedPassword = decryptBytes(key, value);
