...
public class CipherStorageFacebookConceal implements CipherStorage {
    ...
    public EncryptionResult encrypt(@NonNull String service, @NonNull String key, @NonNull String value, SecurityLevel level) throws CryptoFailedException {
        ...
        try {
            byte[] encryptedUsername = crypto.encrypt(key.getBytes(Charset.forName("UTF-8")), usernameEntity);
            byte[] encryptedPassword = crypto.encrypt(value.getBytes(Charset.forName("UTF-8")), passwordEntity);
    ...
    @Override
    public DecryptionResult decrypt(@NonNull String service, @NonNull byte[] key, @NonNull byte[] value) throws CryptoFailedException {
        if (!crypto.isAvailable()) {
            throw new CryptoFailedException("Crypto is missing");
        }
        Entity usernameEntity = createUsernameEntity(service);
        ...
        try {
            byte[] decryptedUsername = crypto.decrypt(key, usernameEntity);
            byte[] decryptedPassword = crypto.decrypt(value, passwordEntity);
